...
//    }
    parse_parameters();
    mtrand.seed(mtrand.randInt());
    print_parameters();
    if (!init_problem(clauses, largest_graph)) {
        //it's actually l_False under assumptions
        //but we'll set the real SAT solver deal with that
        if (verbosity) {
            cout << "c [walksat] problem UNSAT under assumptions, returning to main solver" << endl;
        }
        return l_False;
    }
    ...
    while (!found_solution && numtry < walksat_max_runs) {
        numtry++;
        init_for_round();
...
void CryptoWalkSAT::check_make_break() {
...
//        if (value(lit) != l_Undef) {
        val = value(*lit);
//        } else {
//            val = lit_inside_assumptions(lit);
//        }
...
void CryptoWalkSAT::print_parameters()
{
    if (verbosity) {
        ...
        cout << "c [walksat] tries = " << walksat_max_runs << endl;
...
void CryptoWalkSAT::update_and_print_statistics_end_try()
{
    ...
    if (numfalse == 0 && countunsat() != 0) {
        if (verbosity) {
            cout << "ERROR: CryptoWalkSAT -- verification of solution fails!" << endl;
        }
        exit(-1);
    }
}
...
//        cout << "c [walksat] avg flips per second = " << ratio_for_stat(totalflip, totalTime) << endl;
        if (numtry > 0) {
            cout << "c [walksat] final success rate = " << (1.0 / numtry) * 100.0 << endl;
        }
